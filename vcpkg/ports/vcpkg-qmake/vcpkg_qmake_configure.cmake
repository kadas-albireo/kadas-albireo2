...
function(vcpkg_qmake_configure)
  cmake_parse_arguments(
    ...
    ""
    "SOURCE_PATH"
    "QMAKE_OPTIONS;QMAKE_OPTIONS_RELEASE;QMAKE_OPTIONS_DEBUG;OPTIONS;OPTIONS_RELEASE;OPTIONS_DEBUG"
  )
  ...
  if(NOT VCPKG_BUILD_TYPE OR "${VCPKG_BUILD_TYPE}" STREQUAL "debug")
    list(APPEND buildtypes "DEBUG") # Using uppercase to also access the
                                    # detected cmake variables with it
  ...
  function(qmake_append_program var qmake_var value)
    ...
    if(NOT correct_prog_on_path AND NOT "${value}" MATCHES "|:")
      message(
        FATAL_ERROR
          "Detect path mismatch for '${qmake_var}'. '${value}' is not the same as '${${qmake_var}}'. Please correct your PATH!"
      )
    endif()
    unset(${qmake_var})
  ...
  if(NOT VCPKG_QMAKE_COMMAND) # For users using outside Qt6
    set(VCPKG_QMAKE_COMMAND
        "${CURRENT_HOST_INSTALLED_DIR}/tools/Qt6/bin/qmake${VCPKG_HOST_EXECUTABLE_SUFFIX}"
    )
  endif()
  ...
  foreach(buildtype IN LISTS buildtypes)
    ...
    qmake_add_flags(
      "QMAKE_LIBS"
      "+="
      "${VCPKG_DETECTED_CMAKE_C_STANDARD_LIBRARIES} ${VCPKG_DETECTED_CMAKE_CXX_STANDARD_LIBRARIES}"
    )
    ...
    if(NOT VCPKG_QT_CONF_${buildtype})
      ...
      # configured so users might use the variables within this scope.
      set(qmake_build_tools "")
      set(qmake_comp_flags "")
    endif()
    ...
    # Options might need to go into a response file? I am a bit concerned about
    # cmd line length.
    vcpkg_execute_required_process(
      COMMAND
      ${VCPKG_QMAKE_COMMAND}
      ${qmake_config_${buildtype}}
      ${arg_QMAKE_OPTIONS}
      ${arg_QMAKE_OPTIONS_${buildtype}}
      ${VCPKG_QMAKE_OPTIONS}
      ${VCPKG_QMAKE_OPTIONS_${buildtype}} # Advanced users need a way to inject
                                          # QMAKE variables via the triplet.
      ${qmake_build_tools}
      ${qmake_comp_flags}
      "${arg_SOURCE_PATH}"
      -qtconf
      ...
      ${options}
      WORKING_DIRECTORY
      "${CURRENT_BUILDTREES_DIR}/${config_triplet}"
      LOGNAME
