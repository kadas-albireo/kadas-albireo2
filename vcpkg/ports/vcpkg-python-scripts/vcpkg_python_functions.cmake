...
function(vcpkg_python_build_wheel)
  ...
  # Kadas (overlay fix): let pkg-config-based wheel builds (e.g. py-lxml) find
  # vcpkg-provided libraries such as libxml2/libxslt. Upstream does not put
  # vcpkg_installed/lib/pkgconfig on PKG_CONFIG_PATH, so py-lxml's pkg-config
  # probe fails with "make sure the libxml2 and libxslt development packages are
  # installed" (Ubuntu 22.04 has no system libxslt1-dev either).
  if(VCPKG_TARGET_IS_LINUX OR VCPKG_TARGET_IS_OSX)
    if(DEFINED ENV{PKG_CONFIG_PATH} AND NOT "$ENV{PKG_CONFIG_PATH}" STREQUAL "")
      set(ENV{PKG_CONFIG_PATH}
          "${CURRENT_INSTALLED_DIR}/lib/pkgconfig:$ENV{PKG_CONFIG_PATH}"
      )
    else()
...
function(vcpkg_python_test_import)
  ...
  # Kadas (overlay fix): the import smoke-test must also find the freshly-built
  # *dependencies* in vcpkg_installed/lib (e.g. gdal's libjpeg.so.62), not only
  # the port's own staging lib. Linux RUNPATH does not resolve transitively, so
  # we append CURRENT_INSTALLED_DIR/lib. Without this, gdal[python] fails its
  # post-build import with "libjpeg.so.62: cannot open shared object file".
