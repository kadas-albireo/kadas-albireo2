...
public:
  ...
  enum class Level { Major, Minor, OnlyLabels };
  ...
  struct Grid {
    ...
    friend Grid &operator<<(Grid &lhs, const Grid &rhs) SIP_SKIP {
      ...
      lhs.zoneLabels100kmSquareIdentifier
          << rhs.zoneLabels100kmSquareIdentifier;
      lhs.lines << rhs.lines;
  ...
  enum class GridMode SIP_MONKEYPATCH_SCOPEENUM { GridUTM, GridMGRS };

  static Grid computeGrid(const QgsRectangle &bbox, double mapScale,
...
private:
  ...
  static Grid computeSubGrid(int cellSize, Level level, double xMin,
                             double xMax, double yMin, double yMax,
                             zoneLabelCallback_t *zoneLabelCallback = nullptr,
                             gridLabelCallback_t *lineLabelCallback = nullptr);
  static ZoneLabel mgrs100kIDLabelCallback(double posX, double posY,
                                           double maxLon, double maxLat);
  static GridLabel utmGridLabelCallback(double lon, double lat, int cellSize,
                                        bool horiz, int lineIdx);
  static GridLabel mgrsGridLabelCallback(double lon, double lat, int cellSize,
                                         bool horiz, int lineIdx);
};
