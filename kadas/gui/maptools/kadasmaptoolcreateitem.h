...
class KADAS_GUI_EXPORT KadasMapToolCreateItem : public QgsMapTool {
  Q_OBJECT
public:
  KadasMapToolCreateItem(QgsMapCanvas *canvas,
                         std::unique_ptr<KadasMapItemInterface> interface,
                         KadasItemLayer *layer = nullptr) SIP_SKIP;

  KadasMapToolCreateItem(QgsMapCanvas *canvas,
                         KadasMapItemInterface *interface SIP_TRANSFER,
                         KadasItemLayer *layer = nullptr);

  KadasMapToolCreateItem(QgsMapCanvas *canvas, KadasMapItem *item,
                         KadasItemLayer *layer = nullptr);
  ...
  void setUndoRedoVisible(bool undoRedoVisible) {
    mUndoRedoVisible = undoRedoVisible;
  }
...
#ifndef SIP_RUN
  void
  showLayerSelection(bool enabled, QgsLayerTreeView *layerTreeView,
                     KadasLayerSelectionWidget::LayerFilter filter,
                     KadasLayerSelectionWidget::LayerCreator creator = nullptr);
#else
  // TODO: creator should be optional
  void showLayerSelection(bool enabled, QgsLayerTreeView *layerTreeView,
                          SIP_PYCALLABLE filter, SIP_PYCALLABLE creator);
  // clang-format off
    % MethodCode
  ...
  // clang-format on
#endif
  void addPartFromGeometry(const QgsAbstractGeometry &geom,
