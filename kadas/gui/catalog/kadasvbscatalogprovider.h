...
private:
  struct ResultEntry {
    ...
    ResultEntry(const QString &_category, const QString &_title,
        ...
        : category(_category), title(_title), sortIndices(_sortIndices),
          metadataUrl(_metadataUrl), flatten(_flatten) {}
    ResultEntry(const ResultEntry &entry)
        : category(entry.category), title(entry.title),
          sortIndices(entry.sortIndices), metadataUrl(entry.metadataUrl) {}
    QString category;
    QString title;
    QString sortIndices;
    QString metadataUrl;
  ...
  void searchMatchingWMSLayer(const QDomNode &layerItem,
                              const EntryMap &entries, const QString &url,
