...
class KADAS_GUI_EXPORT KadasMilxItem : public KadasMapItem {
  ...
  Q_PROPERTY(
      bool hasVariablePoints READ hasVariablePoints WRITE setHasVariablePoints)
...
public:
  ...
  bool hitTest(const KadasMapPos &pos,
  ...
  static KadasMilxItem *fromMilx(const QDomElement &itemElement,
                                 const QgsCoordinateTransform &crst,
                                 int symbolSize);
  static KadasMilxItem *
  fromMssStringAndPoints(const QString &mssString,
                         const QList<KadasItemPos> &points);

  static QRect computeScreenExtent(const QgsRectangle &mapExtent,
                                   const QgsMapToPixel &mapToPixel);

  static bool validateMssString(const QString &mssString,
                                QString &adjustedMssString SIP_OUT,
                                QString &messages SIP_OUT);

protected:
  KadasMapItem *_clone() const override SIP_FACTORY {
    return new KadasMilxItem();
  }
  KadasMilxItem::State *createEmptyState() const override SIP_FACTORY {
    return new State();
  }
...
private:
  ...
  QList<QPoint>
  computeScreenPoints(const QgsMapToPixel &mapToPixel,
                      const QgsCoordinateTransform &mapCrst) const;
  QList<QPair<int, double>>
  computeScreenAttributes(const QgsMapToPixel &mapToPixel,
                          const QgsCoordinateTransform &mapCrst) const;
  KadasMilxClient::NPointSymbol
  toSymbol(const QgsMapToPixel &mapToPixel,
           const QgsCoordinateReferenceSystem &mapCrs,
           bool colored = true) const;
  double metersToPixels(const QgsPointXY &refPoint,
                        const QgsMapToPixel &mapToPixel,
                        const QgsCoordinateTransform &mapCrst) const;
  void updateSymbol(const QgsMapSettings &mapSettings,
                    const KadasMilxClient::NPointSymbolGraphic &result);
  ...
  static void posPointNodeRenderer(QPainter *painter,
                                   const QPointF &screenPoint, int nodeSize);
  static void ctrlPointNodeRenderer(QPainter *painter,
                                    const QPointF &screenPoint, int nodeSize);
};
