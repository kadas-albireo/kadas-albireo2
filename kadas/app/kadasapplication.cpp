...
const QgsSettingsEntryBool *KadasApplication::settingsDisableHttp2
  = new QgsSettingsEntryBool( QStringLiteral( "disable-http2" ), KadasSettingsTree::sTreeKadas, true, QStringLiteral( "Force HTTP/1.1 on all outgoing requests (workaround for QTBUG-143926: QNAM hangs on IWA/Negotiate-protected endpoints over HTTP/2). Drop the default once Qt fixes HTTP/2 + Negotiate auth retry." ) );
...
bool KadasApplication::projectSave( const QString &fileName, bool promptFileName )
{
  ...
  {
    QMessageBox::critical( mMainWindow, tr( "Unable to save project %1" ).arg( QDir::toNativeSeparators( QgsProject::instance()->fileName() ) ), QgsProject::instance()->error() );
  }
  return success;
}
...
{
  // HTTP/2 workaround for QTBUG-143926: QNAM hangs on IWA/Negotiate-protected
  // endpoints when the server negotiates HTTP/2 via ALPN — the 401 challenge is
  // received but the authenticationRequired retry loop never fires and the reply
  // never emits finished. Forcing HTTP/1.1 restores the Qt 5 behaviour
  // (transparent SSPI/SSO on Windows). Drop this once Qt fixes HTTP/2 +
  // Negotiate auth retry.
