...
public:
  ...
  void setup(const QgsRectangle &gridRect, int cols, int rows,
             const QgsCoordinateReferenceSystem &crs, bool colSizeLocked,
             bool rowSizeLocked);

  QString layerTypeKey() const override { return layerType(); }
  KadasGuideGridLayer *clone() const override;
  QList<IdentifyResult> identify(const QgsPointXY &mapPos,
  ...
  QPair<QChar, QChar> labelingMode() const {
    return qMakePair(mGridConfig.rowChar, mGridConfig.colChar);
  }
  LabelingPos labelingPos() const { return mGridConfig.labelingPos; }
  QuadrantLabeling labelQuadrants() const {
    return mGridConfig.quadrantLabeling;
  }
...
public:
  ...
  QgsPluginLayer *createLayer(const QString &uri) override {
    return new KadasGuideGridLayer("");
  }
