...
public:
  ...
  KadasMapToolGuideGrid(QgsMapCanvas *canvas, QgsLayerTreeView *layerTreeView,
                        QgsMapLayer *layer);
...
public:
  KadasGuideGridWidget(QgsMapCanvas *canvas, QgsLayerTreeView *layerTreeView,
                       QgsMapLayer *layer);
  void pointPicked(KadasMapToolGuideGrid::PickMode pickMode,
                   const QgsPointXY &pos);
...
private slots:
  ...
  void updateLabeling();
  void pickTopLeftPos() {
    emit requestPick(KadasMapToolGuideGrid::PICK_TOP_LEFT);
  }
  void pickBottomRight() {
    emit requestPick(KadasMapToolGuideGrid::PICK_BOTTOM_RIGHT);
  }
};
