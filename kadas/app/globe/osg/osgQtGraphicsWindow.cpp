...
osgQtGLWidget::osgQtGLWidget( QWidget *parent, const QGLWidget *shareWidget, Qt::WindowFlags f, bool forwardKeyEvents )
...
osgQtGLWidget::osgQtGLWidget( const QGLFormat &format, QWidget *parent, const QGLWidget *shareWidget, Qt::WindowFlags f,
...
osgQtGLWidget::~osgQtGLWidget()
{
  // close osgQtGraphicsWindow and remove the reference to us
  if ( _gw )
  {
    _gw->close();
    _gw->_widget = nullptr;
...
void osgQtGLWidget::setTouchEventsEnabled( bool e )
{
...
void osgQtGLWidget::processDeferredEvents()
{
...
bool osgQtGLWidget::event( QEvent *event )
{
...
void osgQtGLWidget::setKeyboardModifiers( QInputEvent *event )
{
...
void osgQtGLWidget::resizeEvent( QResizeEvent *event )
{
...
void osgQtGLWidget::moveEvent( QMoveEvent *event )
{
  if ( _gw == NULL || !_gw->valid() )
    return;
...
void osgQtGLWidget::glDraw()
{
  _gw->requestRedraw();
}

void osgQtGLWidget::keyPressEvent( QKeyEvent *event )
{
...
void osgQtGLWidget::keyReleaseEvent( QKeyEvent *event )
{
...
void osgQtGLWidget::mousePressEvent( QMouseEvent *event )
{
  int button = 0;
  switch ( event->button() )
  {
...
void osgQtGLWidget::mouseReleaseEvent( QMouseEvent *event )
{
  int button = 0;
  switch ( event->button() )
  {
...
void osgQtGLWidget::mouseDoubleClickEvent( QMouseEvent *event )
{
  int button = 0;
  switch ( event->button() )
  {
...
void osgQtGLWidget::mouseMoveEvent( QMouseEvent *event )
{
  setKeyboardModifiers( event );
  _gw->getEventQueue()->mouseMotion( event->x()*_devicePixelRatio, event->y()*_devicePixelRatio );
}

void osgQtGLWidget::wheelEvent( QWheelEvent *event )
{
  setKeyboardModifiers( event );
...
bool osgQtGLWidget::gestureEvent( QGestureEvent *qevent )
{
  bool accept = false;
...
osgQtGraphicsWindow::osgQtGraphicsWindow( osg::GraphicsContext::Traits *traits, QWidget *parent, const QGLWidget *shareWidget, Qt::WindowFlags f )
  :   _realized( false )
{
...
osgQtGraphicsWindow::~osgQtGraphicsWindow()
{
  close();

  // remove reference from osgQtGLWidget
  if ( _widget )
    _widget->_gw = nullptr;
}

bool osgQtGraphicsWindow::init( QWidget *parent, const QGLWidget *shareWidget, Qt::WindowFlags f )
{
  ...
  if ( !_widget )
  {
    ...
    if ( !shareWidget )
    {
      osgQtGraphicsWindow *sharedContextQt = dynamic_cast<osgQtGraphicsWindow *>( _traits->sharedContext.get() );
      if ( sharedContextQt )
        shareWidget = sharedContextQt->_widget;
    }
    ...
    // create widget
    _widget = new osgQtGLWidget( traits2qglFormat( _traits.get() ), parent, shareWidget, flags );
  }
...
QGLFormat osgQtGraphicsWindow::traits2qglFormat( const osg::GraphicsContext::Traits *traits )
{
...
void osgQtGraphicsWindow::qglFormat2traits( const QGLFormat &format, osg::GraphicsContext::Traits *traits )
{
...
bool osgQtGraphicsWindow::setWindowRectangleImplementation( int x, int y, int width, int height )
{
  if ( _widget == nullptr )
    return false;
...
void osgQtGraphicsWindow::getWindowRectangle( int &x, int &y, int &width, int &height )
{
  if ( _widget )
  {
...
bool osgQtGraphicsWindow::setWindowDecorationImplementation( bool windowDecoration )
{
...
bool osgQtGraphicsWindow::getWindowDecoration() const
{
  return _traits->windowDecoration;
}

void osgQtGraphicsWindow::grabFocus()
{
  if ( _widget )
    _widget->setFocus( Qt::ActiveWindowFocusReason );
}

void osgQtGraphicsWindow::grabFocusIfPointerInWindow()
{
  if ( _widget->underMouse() )
    _widget->setFocus( Qt::ActiveWindowFocusReason );
}

void osgQtGraphicsWindow::raiseWindow()
{
  if ( _widget )
    _widget->raise();
}

void osgQtGraphicsWindow::setWindowName( const std::string &name )
{
  if ( _widget )
    _widget->setWindowTitle( name.c_str() );
}

std::string osgQtGraphicsWindow::getWindowName()
{
  return _widget ? _widget->windowTitle().toStdString() : "";
}

void osgQtGraphicsWindow::useCursor( bool cursorOn )
{
  if ( _widget )
  {
...
void osgQtGraphicsWindow::setCursor( MouseCursor cursor )
{
...
bool osgQtGraphicsWindow::valid() const
{
  return _widget && _widget->isValid();
}

bool osgQtGraphicsWindow::realizeImplementation()
{
...
bool osgQtGraphicsWindow::isRealizedImplementation() const
{
  return _realized;
}

void osgQtGraphicsWindow::closeImplementation()
{
  if ( _widget )
    _widget->close();
  _realized = false;
...
void osgQtGraphicsWindow::runOperations()
{
...
bool osgQtGraphicsWindow::makeCurrentImplementation()
{
  if ( _widget->getNumDeferredEvents() > 0 )
    _widget->processDeferredEvents();
...
bool osgQtGraphicsWindow::releaseContextImplementation()
{
  _widget->doneCurrent();
  return true;
}

void osgQtGraphicsWindow::swapBuffersImplementation()
{
...
void osgQtGraphicsWindow::requestWarpPointer( float x, float y )
{
  if ( _widget )
...
class QtWindowingSystem : public osg::GraphicsContext::WindowingSystemInterface
{
  public:
    ...
    virtual osg::GraphicsContext *createGraphicsContext( osg::GraphicsContext::Traits *traits )
    {
      ...
      {
        osg::ref_ptr< osgQtGraphicsWindow > window = new osgQtGraphicsWindow( traits );
