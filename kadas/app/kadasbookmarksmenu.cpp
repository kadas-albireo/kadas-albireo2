...
void KadasBookmarksMenu::addBookmark()
{
  ...
  for ( QList<Bookmark *>::iterator it = mBookmarks.begin(); it != mBookmarks.end(); it++ )
  {
    ...
    {
      int res = QMessageBox::
        question( kApp->mainWindow(), tr( "Replace Bookmark" ), tr( "Are you sure you want to replace the existing bookmark “%1”?" ).arg( name ), QMessageBox::Yes | QMessageBox::No, QMessageBox::No );
...
void KadasBookmarksMenu::saveToProject( QDomDocument &doc )
{
  ...
  for ( const Bookmark *bookmark : std::as_const( mBookmarks ) )
  {
    ...
    bookmarkEl.setAttribute(
      "extent",
      QString( "%1;%2;%3;%4" ).arg( bookmark->extent.xMinimum(), 0, 'f', 4 ).arg( bookmark->extent.yMinimum(), 0, 'f', 4 ).arg( bookmark->extent.xMaximum(), 0, 'f', 4 ).arg( bookmark->extent.yMaximum(), 0, 'f', 4 )
    );
