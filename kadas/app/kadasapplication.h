...
public:
  ...
  KadasPythonIntegration *pythonIntegration() const {
    return mPythonIntegration;
  }
  KadasLayerRefreshManager *layerRefreshManager() const {
    return mLayerRefreshManager;
  }

  QgsRasterLayer *addRasterLayer(const QString &uri, const QString &baseName,
                                 const QString &providerKey, bool quiet = false,
                                 int insOffset = 0,
                                 bool adjustInsertionPoint = true) const;
  QgsVectorLayer *addVectorLayer(const QString &uri, const QString &layerName,
                                 const QString &providerKey, bool quiet = false,
                                 int insOffset = 0,
                                 bool adjustInsertionPoint = true) const;
  void addVectorLayers(const QStringList &layerUris, const QString &enc,
                       const QString &dataSourceType, bool quiet = false) const;
  void addRasterLayers(const QStringList &layerUris, bool quiet = false) const;
  QgsVectorTileLayer *addVectorTileLayer(const QString &url,
                                         const QString &baseName,
                                         bool quiet = false,
                                         bool forceUpdateUriSources = true);
  QgsPointCloudLayer *addPointCloudLayer(const QString &uri,
                                         const QString &baseName,
                                         const QString &providerKey,
                                         bool quiet = false);
  QPair<KadasMapItem *, KadasItemLayerRegistry::StandardLayer>
  addImageItem(const QString &filename) const;
  KadasItemLayer *
  selectPasteTargetItemLayer(const QList<KadasMapItem *> &items);
  bool
  askUserForDatumTransform(const QgsCoordinateReferenceSystem &sourceCrs,
  ...
  void projectClose();
  bool projectSave(const QString &fileName = QString(),
                   bool promptFileName = false);
  ...
  void
  registerMapLayerPropertiesFactory(QgsMapLayerConfigWidgetFactory *factory);
  void
  unregisterMapLayerPropertiesFactory(QgsMapLayerConfigWidgetFactory *factory);
...
private:
  ...
  void mergeChildSettingsGroups(QgsSettings &settings,
