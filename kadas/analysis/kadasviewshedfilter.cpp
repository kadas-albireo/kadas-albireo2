...
bool KadasViewshedFilter::computeViewshed(
  ...
  QgsPointXY observerPos,
  const QgsCoordinateReferenceSystem &observerPosCrs,
  double observerHeight,
  double targetHeight,
  bool observerHeightRelToTerr,
  bool targetHeightRelToTerr,
  double observerMinVertAngle,
  double observerMaxVertAngle,
  double radius,
  const Qgis::DistanceUnit distanceElevUnit,
  QProgressDialog *progress,
  QString *errMsg,
  const QVector<QgsPointXY> &filterRegion,
  int accuracyFactor
)
{
  // Open input file
  ...
  int obs[2] = { qRound( geoToPixelX( gtrans, observerPos.x(), observerPos.y() ) ), qRound( geoToPixelY( gtrans, observerPos.x(), observerPos.y() ) ) };
