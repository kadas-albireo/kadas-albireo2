...
public:
   ...
   * string)*/
  KadasNineCellFilter(const QgsRasterLayer *layer, const QString &outputFile,
  ...
  /**Starts the calculation, reads from mInputFile and stores the result in
    mOutputFile
    @param p progress dialog that receives update and that is checked for abort.
    0 if no progress bar is needed.
  ...
  //! output value can be equal to the nodata value if not present or outside of
  //! the border. Must be implemented by subclasses
  virtual float processNineCellWindow(float *x11, float *x21, float *x31,
                                      float *x12, float *x22, float *x32,
                                      float *x13, float *x23, float *x33) = 0;

  //! Computes the window of the raster which contains the specified region of
  //! the raster
  static bool computeWindow(GDALDatasetH dataset,
                            const QgsCoordinateReferenceSystem &datasetCrs,
                            const QgsRectangle &region,
                            const QgsCoordinateReferenceSystem &regionCrs,
                            int &rowStart, int &rowEnd, int &colStart,
                            int &colEnd);

private:
  /**Opens the input file and returns the dataset handle and the number of
   * pixels in x-/y- direction*/
  GDALDatasetH openInputFile(int &nCellsX, int &nCellsY);
  /**Opens the output driver and tests if it supports the creation of a new
    dataset
    @return NULL on error and the driver handle on success*/
  GDALDriverH openOutputDriver();
  /**Opens the output file and sets the same geotransform and CRS as the input
    data
    @return the output dataset or NULL in case of error*/
  GDALDatasetH openOutputFile(GDALDatasetH inputDataset,
                              const QgsCoordinateReferenceSystem &inputCrs,
                              GDALDriverH outputDriver, int colStart,
                              int rowStart, int xSize, int ySize);

protected:
  /**Calculates the first order derivative in x-direction according to Horn
   * (1981)*/
  float calcFirstDerX(float *x11, float *x21, float *x31, float *x12,
                      float *x22, float *x32, float *x13, float *x23,
                      float *x33);
  /**Calculates the first order derivative in y-direction according to Horn
   * (1981)*/
  float calcFirstDerY(float *x11, float *x21, float *x31, float *x12,
                      float *x22, float *x32, float *x13, float *x23,
                      float *x33);
  ...
  /**Scale factor for z-value if x-/y- units are different to z-units (111120
   * for degree->meters and 370400 for degree->feet)*/
  double mZFactor = -1.0;
};
