    ...
    for interval, feature in zip(intervals[::-1], features):
        ...
        try:
            ...
            if overwrite:
                ...
                raise OverwriteError('layer {} already exists and overwrite is {}'.format(layername, overwrite))
        except IndexError:
            LOG.debug('this layer was not found: {}'.format(layername))            
        
        layer = QgsVectorLayer("Polygon?crs=epsg:4326&field=centerx:double&field=centery:double&field=interval:double",
                                layername, "memory")
