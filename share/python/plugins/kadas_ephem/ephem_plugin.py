...
class EphemPlugin:
    ...
    def __init__(self, iface):
        ...
        locale_path = os.path.join(
            self.plugin_dir, "i18n", "kadas_ephem_{}.qm".format(self.locale)
        )
    ...
    def initGui(self):
        ...
        self.action = QAction(QIcon(icon_path), self.tr("Ephemeris"), self.iface.mainWindow())
        self.action.setCheckable(True)
        self.action.toggled.connect(self.toolToggled)
        self.iface.addAction(self.action, self.iface.PLUGIN_MENU, self.iface.ANALYSIS_TAB)

        icon_dark_path = os.path.join(os.path.dirname(__file__), "icons/icon_color.svg")
        self.actionCanvasRightClick = QAction(
            QIcon(icon_dark_path), self.action.text(), self.iface.mainWindow()
        )
    ...
    def unload(self):
        self.iface.removeAction(self.action, self.iface.PLUGIN_MENU, self.iface.ANALYSIS_TAB)
