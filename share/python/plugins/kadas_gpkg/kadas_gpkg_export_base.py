        ...
        for layerid in selected_layers:
            ...
            if layer.type() == QgsMapLayer.VectorLayer:
                ...
                if ret[0] == 0:
                    ...
                    messages.append(
                        "%s: %s"
                        % (layer.name(), self.tr("Write failed: error %d (%s)") % (ret[0], ret[1]))
                    )
                ...
                projector.setCrs(
                    provider.crs(), provider.crs(), QgsProject.instance().transformContext()
                )
                ...
                ret = writer.writeRaster(pipe, cols, rows, exportExtent, provider.crs(), feedback)
                if ret == 0:
                    added_layer_ids.append(layerid)
                    added_layers_by_source[layer.source()] = layerid
                else:
                    messages.append(
                        "%s: %s" % (layer.name(), self.tr("Write failed: error %d") % ret)
                    )
