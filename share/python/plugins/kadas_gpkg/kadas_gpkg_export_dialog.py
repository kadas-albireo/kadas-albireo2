...
WidgetUi, _ = loadUiType(os.path.join(os.path.dirname(__file__), "kadas_gpkg_export_dialog.ui"))
...
class KadasGpkgExportDialog(QDialog, WidgetUi):
    ...
    def __updateLocalLayerList(self):
        self.listWidgetLayers.updateLayerList(
            self.lineEditOutputFile.text() if not self.checkBoxClear.isChecked() else None
        )
