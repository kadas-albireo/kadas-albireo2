...
if(WIN32)
  execute_process(
    COMMAND cmd /C "${GIT_EXECUTABLE} -C ${VCPKG_ROOT} rev-parse HEAD"
    OUTPUT_VARIABLE VCPKG_VERSION
    OUTPUT_STRIP_TRAILING_WHITESPACE)
else()
  execute_process(
    COMMAND bash -c "${GIT_EXECUTABLE} -C ${VCPKG_ROOT} rev-parse HEAD"
    OUTPUT_VARIABLE VCPKG_VERSION
    ERROR_VARIABLE ERR
    OUTPUT_STRIP_TRAILING_WHITESPACE)
endif()
...
message(
  STATUS "Building with vcpkg libraries version ${VCPKG_INSTALLED_VERSION}")

# Binarycache can only be used on Windows or if mono is available.
find_program(_VCPKG_MONO mono)
if(NOT "${NUGET_TOKEN}" STREQUAL "" AND (_HOST_IS_WINDOWS OR EXISTS
                                                             "${_VCPKG_MONO}"))
  # Early bootstrap, copied from the vcpkg toolchain, we need this to fetch
  # nuget
  if(_HOST_IS_WINDOWS)
  ...
  if(NOT EXISTS "${_VCPKG_EXECUTABLE}")
    ...
    file(TO_NATIVE_PATH "${CMAKE_BINARY_DIR}/vcpkg-bootstrap.log"
         _VCPKG_BOOTSTRAP_LOG)
    execute_process(
    ...
    if(_VCPKG_BOOTSTRAP_RESULT EQUAL 0)
      ...
      message(
        FATAL_ERROR
          "vcpkg install failed. See logs for more information: ${MSG}")
    endif()
  endif()
  ...
  configure_file("${CMAKE_SOURCE_DIR}/cmake/NuGet.Config.in" "${_CONFIG_PATH}"
                 @ONLY)
  execute_process(
    COMMAND
      ${_NUGET_EXE} setapikey "${NUGET_TOKEN}" -src
      "https://nuget.pkg.github.com/kadas-albireo/index.json" -configfile
      ${_CONFIG_PATH}
